...
// 约束天空盒索引，避免 UI 状态被非法值污染；当无候选项时返回 -1。
int ClampSkyboxIndex(int skyboxIndex, int optionCount)
{
    if (optionCount <= 0) {
        return -1;
    }
    return std::clamp(skyboxIndex, 0, optionCount - 1);
}
}
...
void DebugUI::drawSceneSection(Scene& scene)
{
...
void DebugUI::drawSkyboxTab(Scene& scene)
{
    ...
    if (!skyboxOptions_.empty()) {
        ...
        skyboxItems.reserve(skyboxOptions_.size());
        for (const std::string& optionName : skyboxOptions_) {
            skyboxItems.push_back(optionName.c_str());
        }

        if (ImGui::Combo("Skybox", &skyboxIndex, skyboxItems.data(), static_cast<int>(skyboxItems.size()))) {
            skyboxIndex = ClampSkyboxIndex(skyboxIndex, static_cast<int>(skyboxOptions_.size()));
            if (skyboxIndex != currentSkyboxIndex_) {
                currentSkyboxIndex_ = skyboxIndex;
...
void DebugUI::drawShadowTab(Scene& scene)
{
...
void DebugUI::drawModelTab(Scene& scene)
{
    if (scene.objects.empty()) {
...
void DebugUI::drawStatusTab(const Scene& scene)
{
...
void DebugUI::setSkyboxOptions(const std::vector<std::string>& options)
{
    skyboxOptions_ = options;
    pendingSkyboxIndex_ = -1;

    if (skyboxOptions_.empty()) {
        currentSkyboxIndex_ = -1;
        return;
    }

    currentSkyboxIndex_ = ClampSkyboxIndex(currentSkyboxIndex_, static_cast<int>(skyboxOptions_.size()));
    if (currentSkyboxIndex_ < 0) {
        currentSkyboxIndex_ = 0;
    }
}

void DebugUI::setCurrentSkyboxIndex(int skyboxIndex)
{
    currentSkyboxIndex_ = ClampSkyboxIndex(skyboxIndex, static_cast<int>(skyboxOptions_.size()));
    if (!skyboxOptions_.empty() && currentSkyboxIndex_ < 0) {
        currentSkyboxIndex_ = 0;
    }
    pendingSkyboxIndex_ = -1;
}

bool DebugUI::hasPendingSkyboxSwitch() const
{
    return pendingSkyboxIndex_ >= 0;
}

int DebugUI::consumePendingSkyboxSwitch()
{
    const int requestedSkyboxIndex = pendingSkyboxIndex_;
    pendingSkyboxIndex_ = -1;
    return requestedSkyboxIndex;
}
