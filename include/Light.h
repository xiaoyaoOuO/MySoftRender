...
protected:
    ...
    // 缓存阴影计算所需的光源观察矩阵、投影矩阵和组合矩阵。外部通过 lightSpaceMatrix() 读取，供 ShadowMap pass 与着色阶段采样坐标变换使用。
    ...
    // 控制方向光阴影正交体与深度范围。按场景尺度调节半径/近平面/远平面，以平衡覆盖范围与深度精度。
...
public:
    ...
    // 更新光源基础属性，并在必要时刷新阴影矩阵缓存。运行时改光源位置/方向时调用，ShadowMap 会在下一帧使用新矩阵。
    ...
    // 访问光源视角深度缓存（ShadowMap 深度图）。第一个 Pass 写入该缓存，第二个 Pass 可按需读取进行阴影判断。
    ...
    // 记录阴影贴图实际尺寸，保证采样阶段使用正确的宽高做坐标映射与边界检查。在第一 Pass 分配/重建阴影深度图后调用，第二 Pass 读取该尺寸进行采样。
    ...
    // 根据当前光源参数重建阴影矩阵。通常由 setPosition/setDirection/setType 或阴影范围参数修改后自动调用。
    void updateShadowMatrices();

    // 设置点光源阴影每个面的分辨率。传入正整数分辨率，阴影 pass 会按该尺寸分配 6 面深度缓存。
    void setPointShadowResolution(int resolution);
    int pointShadowResolution() const { return pointShadowResolution_; }

    // 访问点光源阴影某个面的光空间矩阵。Shadow Pass 与采样阶段按面索引（0~5）读取矩阵。
    const glm::mat4& pointLightSpaceMatrix(std::size_t faceIndex) const;

    // 访问点光源阴影某个面的深度缓存。第一个 Pass 写入，第二个 Pass 按面索引读取。
